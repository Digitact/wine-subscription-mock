...
const FlowHeader = ({steps, currentStep, goToStep}) => {
    let rows = []
        ...
        if (steps[i].visible) {
            rows.push(
                <Col className={style} onClick={(e) => { e.preventDefault(); goToStep(i, done, style); } }>
                    ...
                    {stepnum}
                    {style === 'complete-step' ? <><img className="mx-2"  style={{'vertical-align':'baseline'}} alt="Tick" src={window.asset_url + Tick} width={26} height={26}/></> : null}
                    </h3>
