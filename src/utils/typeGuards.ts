 ...
 */
export function isArrayOfType<T>(o: unknown | undefined, typeguard: TypeGuard<T>): o is Array<T> {
    return o != null && Array.isArray(o) && o.reduce((prev: boolean, i: unknown) => prev && typeguard(i), true);
}
/**
...
export function isKeyOfObject<T extends object>(x: string | number | symbol, obj: T): x is keyof T {
    return x in obj;
}
 ...
 */
export function getObjectKeys<T extends { [key: string]: unknown }>(obj: T): ReadonlyArray<keyof T> {
 ...
 * @param x Value to verify
 */
export function isNullishOrType<T>(guard: TypeGuard<T>, x: unknown): x is T | null | undefined {
